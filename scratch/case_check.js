...
for (let id = 1; id <= 8; id++) {
    ...
    // Read the actual directory contents (recursively if needed, but we can do a flat mapping for non-nested folders)
    const dirPath = path.join(BASE_DIR, 'images', folder);
    if (!fs.existsSync(dirPath)) {
        console.log(`[DIRECTORY MISSING] images/${folder}`);
        mismatchCount++;
        continue;
    }
    ...
    function getFiles(dir, prefix = '') {
        ...
        list.forEach(file => {
            ...
            const relative = prefix ? prefix + '/' + file : file;
            if (stat.isDirectory()) {
                results = results.concat(getFiles(full, relative));
            } else {
    ...
    const actualFiles = getFiles(dirPath);
    
    // Create a Set of actual files for O(1) case-sensitive lookup
    const actualSet = new Set(actualFiles);
    // Create a lowercase map for case-insensitive matches
    const lowercaseMap = new Map(actualFiles.map(f => [f.toLowerCase(), f]));
    
    images.forEach(filename => {
        totalCount++;
        // Standardize separators in filename for check
        const stdFilename = filename.replace(/\\/g, '/');
        
        if (actualSet.has(stdFilename)) {
            // Perfect case-sensitive match
        } else {
            const matchedCase = lowercaseMap.get(stdFilename.toLowerCase());
            if (matchedCase) {
                console.log(`[CASE MISMATCH] Project ${id} (${folder}): Code lists "${stdFilename}" but disk has "${matchedCase}"`);
                mismatchCount++;
            } else {
                console.log(`[FILE MISSING ON DISK] Project ${id} (${folder}): ${stdFilename}`);
                mismatchCount++;
            }
        }
    });
...
console.log(`\nCase check completed. Total checked: ${totalCount}, Mismatches/Missing: ${mismatchCount}`);
